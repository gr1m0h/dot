...
- **Route by review depth, not task size**: review can be a rubber-stamp → `/batch` (horizontal fan-out, 🟢). Review is itself deep (must understand + hands-on) → `/deepwork` (serial pull, WIP=1, JIT `prefetch` of the next task). On deep work, throughput comes from _warm starts + within-task acceleration_, NOT from parallelising the human.
