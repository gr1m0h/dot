...
- `~/.claude/batch/inbox.md` — task queue. One task per line under `## Queue`:
  `- [ ] <case>: <task> | 成果物: <format> | 完了条件: <criteria>`
  (`成果物`/`完了条件` may be omitted; see Phase 1.)
- `~/.claude/batch/log-YYYY-MM-DD.md` — dispatch log for the day
- `~/.claude/batch/unreported.md` — sessions in tracked orgs (`~/.claude/tracked-orgs.txt`) that ended without leaving a report (appended by the SessionEnd hook; client orgs use the sreaas marker, personal orgs can use reflect/learn)
- `~/.claude/batch/kpi.md` — weekly KPI table (appended by the weekly sreaas draft routine — see `~/.claude/scheduled-tasks/weekly-sreaas-report/`)
...
3. **Dispatch**: for each task, spawn a background subagent (`run_in_background: true`; `general-purpose`, or `Explore` for pure investigation):
   - Prompt = task + 成果物 + 完了条件 + "write the deliverable to `~/.claude/batch/out/YYYY-MM-DD-<case>-<slug>.md`" (for code tasks: work in the case repo, prepare a branch + diff, do NOT push or open PRs)
...
4. Mark dispatched tasks `- [~]` in inbox with agent IDs; write `~/.claude/batch/log-YYYY-MM-DD.md` (task → agent ID → expected artifact path).
5. If `~/.claude/batch/unreported.md` is non-empty, list its entries and propose per-session `/sreaas:task report` (or `/sreaas:task report all` for the whole iteration).
6. Final message: dispatch table (case / task / agent ID / artifact path) + "レビューは昼と夕方に" reminder. Then stop — do not babysit the agents.

## Verb: `status`

Check background task outputs (TaskOutput / artifact files under `~/.claude/batch/out/`), summarize per task: done (artifact path) / running / failed (reason). Mark finished tasks `- [x]` in inbox and move their line to the day's log.
